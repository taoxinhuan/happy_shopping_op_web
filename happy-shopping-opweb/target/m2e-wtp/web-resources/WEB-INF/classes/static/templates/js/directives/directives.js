...
MetronicApp.directive('trees',
    function (RecursionHelper) {
        return {
            ...
            + '<li ng-repeat="son in sonlabel track by $index" ng-if="son && son.privilegeType==\'Menu\'">'
            + '<a href="{{son.url}}">'
            + '<i class="{{son.icon}}"></i>'
            + '<span class="title" ng-bind="son.title"></span><span class="arrow"  ng-if="son.sonIndex[0]"></span></a>'
            + '<trees sonlabel="son.sonIndex" ng-if="son.sonIndex[0]" ></trees>'
            + '</li></ul>',
        ...
        link: function (scope, elem, attr, ngModelCtrl) {
            ...
            ngModelCtrl.$formatters.push(function (modelValue) {
                ...
                if (attr.ejpNumberFilter == '' || !isFinite(modelValue) || angular.isObject(modelValue)) {
                    ...
                    return Number(modelValue).toFixed(attr.ejpNumberFilter);
                }
            });
...
MetronicApp.directive('pageBox', [function () {
    return {
        ...
        replace: true,
        transclude: true,
        templateUrl: "js/directives/pageTemplates/pageBox.html"
    };
}]);
...
MetronicApp.directive('pageTop', [function () {
    return {
        ...
        replace: true,
        transclude: true,
        templateUrl: "js/directives/pageTemplates/pageTop.html"
    };
}]);
...
MetronicApp.directive('pageContent', [function () {
    return {
        ...
        replace: true,
        transclude: true,
        templateUrl: "js/directives/pageTemplates/pageContent.html"
    };
}]);
/**
 *页面搜索框
...
MetronicApp.directive('pageSearch', [function () {
    return {
        ...
        replace: true,
        transclude: true,
        templateUrl: "js/directives/pageTemplates/pageSearch.html",
    };
}]);
 ...
 * 日期选择指令
 */
MetronicApp.directive("myDatepicker", ['$filter', function ($filter) {
    return {
        restrict: 'A',
        require: 'ngModel',
        link: function (scope, element, attr, ctrl) {

            var format = (attr.myDatepicker == '') ? 'yyyy-MM-dd' : attr.myDatepicker;
...
MetronicApp.directive('onlyChangeCounty', [function () {
    ...
    return {
        ...
        template:  '<select class="form-control dist"' +
                    'data-ng-model="cityObj.county" required'+ '>'+
                    '<option value="">请选择</option>'+
                    '<option value="{{item.s}}" ng-repeat="item in coutryList" data-ng-selected="item.s == cityObj.county">{{item.s}}</option>'+
                    '</select>',
        controller: ['$scope','$http', function ($scope,$http) {

            $http.get("../assets/global/plugins/city.min.js")
                .success(function(data) {
                  angular.forEach(data.citylist,function(item){
                      if(item.p == $scope.cityObj.province){
                          angular.forEach(item.c,function(city){
                              if(city.n == $scope.cityObj.city){
                                  $scope.coutryList = city.a;
                              }
                          });
                      }
                  });
...
//字典选择框（暂时有点问题）
MetronicApp.directive("testDirective", function () {
    return {
    	restrict: "AE",
    	require: 'ngModel',
        scope: {
        ...
        template: '<select class="form-control dist" >"' +
        '<option value="">请选择</option>'+
        '<option value="{{item.key}}" ng-repeat="item in redata.itemList" >{{item.value}}</option>'+
        '</select>',
        controller:  function ($scope,$http) {
             $http.get("dictionary/getDictionaryByCode/"+$scope.code)
              .success(function(data) {
               $scope.redata = data.data;
              })
...
//字典枚举显示
MetronicApp.directive("dictionaryEnum", function () {
	return {
		restrict: "AE",
		scope: {
			code: "@",
		...
		template: 
		'<span  ng-repeat="item in redata.itemList" ng-if="item.key==key" >{{item.value}}</span>',
...
//网上的指令
MetronicApp.directive('selectDictionary',function(){
    return {
        restrict:'AE',  
        scope:{
            code: '@',
        ...
        require:'ngModel',
        template:'<select ng-model=ngModel class="form-control dist">'+
                '<option value="" disabled>--{{pholder}}--</option>'+
                '<option ng-repeat="item in itemList" ng-value="item.key">{{item.value}}</option>'+
                '</select>',
        controller:function($scope,$http){
        	$http.get("dictionary/getDictionaryByCode/"+$scope.code)
			.success(function(data) {
        ...
        link:function(scope, elem, attrs, ngModel){
            ...
            scope.ngModelChange = function(value) {
                ...
                scope.onSelect(scope.dictionaryObj[value]);
                console.info(1);
            };
        }
    }
